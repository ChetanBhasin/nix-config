...
A later input requires `confirm` with the current `revision` and `inputId` for unchanged scope. That acknowledgement itself does no filesystem hashing or evidence reset. `revise` can autonomously **add or strengthen** requirements/journeys/artifact checks and add verification inputs while preserving every existing obligation and objective/kind. It invalidates old evidence but retains continuation budgets. For accidental technical bindings use **`recover` below**. Intentional removals, changed outcomes/interface scope, weaker mandatory/artifact flags or replacement of objective/kind still require a **genuine input**, not a tool assertion:
...
## Automatic lossless setup recovery (normal and Auto)

When the agent accidentally declares an oversized root, omits a symlink dependency, or pins malformed/placeholder journey arguments, use **`recover`**, not a user reset or `workflow-scope` request. This operation is model-usable with Auto off or on and requires no confirmation. It resets technical bookkeeping **keeping every obligation**; it never retires work or claims completion.

```json
{"action":"recover","revision":1,"reason":"Accidentally included dependency/build trees; select the actual source and bind the CLI arguments","definition":{"objective":"Greet a named user from the shipped CLI","kind":"implementation","roots":["/absolute/project/src"],"externalInputs":["/absolute/project/bin/greet","/absolute/fixture.json"],"requirements":[{"id":"greeting","mandatory":true,"expected":"Hello, Ada!"}],"journeys":[{"id":"cli-user","scenario":"User invokes greet with Ada","interface":"shipped greet CLI","tool":"bash","input":{"command":"/absolute/project/bin/greet Ada"},"expected":"Hello, Ada!"}]}}
```

Pass the **full current definition**, with only justified technical `roots`, `externalInputs`, journey `tool` or `input` corrections. Copy the current objective/kind and **every** requirement ID/expected/mandatory/artifact flag and journey ID/scenario/interface/expected/artifact flag exactly, even optional obligations. Add/strengthen obligations separately using `revise`. Intentional scope reduction, goal replacement, dropping unfinished work, or weakening outcomes still requires genuine human authorization. A reason is an auditable agent assertion, not permission or semantic proof that the replacement interface is sufficient.

Before adoption, recovery validates registered active journey tool schemas (including rejecting implicit argument coercion), fingerprints the complete replacement with the existing bounds/error handling, and proves retained changed-source coverage. Missing symlink inputs, unreadable/unstable/oversized inputs or unavailable tools fail with actionable diagnostics; **nothing is adopted**. Fix the diagnosed bindings rather than repeat the same reset. An unchanged-binding recovery is rejected.

Coverage uses a start-time per-entry content/mode/symlink baseline plus a durable over-approximation of guarded mutation targets. New/deleted/modified entries are retained across repairs, including ignored files and external inputs. Shell/LSP/unknown permits conservatively retain their whole declared affected roots, even if the command was read-only. Keep these in the new roots or external inputs (a surviving parent for deletions), as well as necessary symlink targets and worktree HEAD coverage. Missing old baselines/failed post-start scans cannot prove narrowing: retain their old coverage or seek human scope authority. A failed initial scan is repairable before guarded work begins; repair oversized setup **before** starting mutations. Legacy snapshots without coverage tracking remain conservative. External editors/undeclared effects remain outside the cooperative guarantee.

Successful recovery appends one ledger snapshot containing `recover-bindings` audit data: reason, old/new binding hashes, obligation hash, old/new revisions, validated workspace revision and retained-coverage hash. Prior definitions/receipts stay in branch history; active evidence and receipt lookup are cleared, and the controller clears old preflights/permits. Revision increments; objective identity, input provenance, disposition, continuation/no-progress counters and writer ownership are retained. **Acceptance stays open.** Rerun the actual declared interface and supply new finalized evidence before `complete`. Recovery must wait until the mutation batch drains and does not acquire, release, steal or bypass a lease.

Preflight rejection adopts nothing. SDK append errors are different: a subscriber can throw **after** the entry is in memory or on disk. Such uncertain persistence faults the live ledger and clears receipts, evidence, preflights and permits; release ownership, inspect the session and reload its authoritative branch before proceeding. Do not retry against old live acceptance state or assume the attempted snapshot was absent. Deleted roots remain structurally restorable (but cannot pass evidence validation) so recovery can retain their surviving parent. Original and lexical worktree identities are retained across symlink replacement; an authorized scope revision rebases its baseline, while autonomous strengthening cannot discard accumulated coverage.

`/workflow reset` remains the owner-TUI-confirmed **scope retirement** boundary, with an unaccepted outcome for unfinished work. It is not necessary for these lossless repair cases. Never synthesize a user authorization or edit stored ledgers to get unstuck.
...
Run `node workflow-recovery-check.mjs` for the deterministic real-interface recovery gate. It launches two isolated Pi SDK sessions with the actual Auto/workflow controllers (`/auto off` and `/auto on`), exercises setup repair, failed repairs, preserved obligations/changed sources, stale evidence and permits, and checks a newly executed CLI before completion. It prints an absolute `/tmp/.../result.json` artifact plus the two PASS lines only after assertions pass. Session/CLI/process artifacts and caches stay outside source roots; no model network or human confirmation is used.
