...
Never point a store-sourced `home.file` at a path an application rewrites at
runtime. Home Manager projects those paths as read-only store symlinks, and
`force = true` silently discards whatever was already there. A
`programs.zen-browser` module owning `~/Library/Application Support/zen/profiles.ini`
stripped the `[Install<hash>]` sections Zen uses to locate its profile; Zen
responded by building a fresh empty profile and stranding the real one — on
2026-08-24 and again on 2026-09-14.

1. `force = true` is allowed only on `config.lib.file.mkOutOfStoreSymlink`, where
   the real data lives outside the store (see `modules/homeManager/pi.nix`).
2. Config an app writes back to is seeded copy-if-absent from an activation
   script, never projected. `seedFile` in `modules/homeManager/maki.nix` is the
   pattern: `[[ ! -e "$target" ]] && install -Dm600 "$source" "$target"`.
3. Read-only store projection is fine only for files the app never writes.
4. When an app owns a file this repo therefore cannot manage, snapshot it before
   `linkGeneration` instead — see `home.activation.snapshotZenIni` in
...
Two agents are configured side by side, sharing the Gruvbox Night palette and
the same operating contract:

- **Pi** (`home/pi/`, `modules/homeManager/pi.nix`): Node.js agent with Lens
  code intelligence, Hashline edits, and named subagent roles. Its writable
  runtime under `~/.pi/agent` is reconciled against the flake snapshot by
  `pi-config`. See `docs/pi.md`.
- **Maki** (`home/maki/`, `modules/homeManager/maki.nix`): Rust TUI agent
  optimized for context spend. Not in nixpkgs; supplied by the `maki` flake
  input through an overlay. Config lives in `~/.config/maki`, with `init.lua`,
  `plugin.toml`, `AGENTS.md`, themes, commands and skills projected read-only
  and `permissions.toml`/`providers.toml` seeded once. See `docs/maki.md`.

Two Lua plugins live in `home/maki/config/lua/`, both projected read-only with
their `require` line generated by the module:

- `roles.lua`: a `role` tool with scout, researcher, reviewer, oracle and
  worker, ported from the Pi subagent table, plus `/profile` and `/roles`.
  `worker` is the only writing role and holds a semaphore of one, which is
  this setup's replacement for Pi's `writer_lease`.
- `rv.lua`: `rv_review` and `rv_note` over [rv](https://github.com/Firaenix/rv),
  built by `packages/rv.nix`, turning a local Jujutsu stack review into the
  agent's task list.

Tool definitions are billed on every request, so measure before adding one:
`maki prompt --tools` prints the definitions the model sees. See the cost table
in `docs/maki.md`.
